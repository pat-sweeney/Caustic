...
export module Imaging.Image.ImageFilter.HarrisCorners;
...
import Imaging.Image.ImageFilter.Sobel;
...
export namespace Caustic
{
    ...
    //**********************************************************************
    // Method: Apply
    // Detects Harris corners in an image and returns an image with the marked
    // corners
    //
    // Parameters:
    ...
    //**********************************************************************
    CRefObj<IImage> CHarrisCornerFilter::Apply(IImage* pImage, ImageFilterParams* pParams)
    {
        ...
        CRefObj<IImageFilter> sobelX = Caustic::CreateSobelFilter(true, 5);
        CRefObj<IImageFilter> sobelY = Caustic::CreateSobelFilter(false, 5);
        CRefObj<IImage> imgSobelX = sobelX->Apply(pImage, nullptr);
        CRefObj<IImage> imgSobelY = sobelY->Apply(pImage, nullptr);
        RGBColor clr[3][3];
        ...
        for (int y = 0; y < (int)h; y++)
        {
            ...
            while (x--)
            {
                ...
                if (pParams->spMask == nullptr || citer1.GetBit())
                {
                    //
                    // Find median value
                    //
    ...
    {
        return CRefObj<IImageFilter>(new CHarrisCornerFilter());
    }
}
