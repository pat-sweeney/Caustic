...
export namespace Caustic
{
    ...
    protected:
        ...
        bool m_enabled;
        uint32_t m_epoch;
        std::map<std::wstring, std::tuple<IGPUPipelineNode*, std::wstring /*textureName*/, std::wstring /*samplerName*/>> m_sourceNodes; // Weak reference to IGPUPipelineNode
        uint32_t m_width; // Width of output generated by this node
        uint32_t m_height; // Height of output generated by this node
    ...
    public:
        CGPUPipelineNodeBase(uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT outputFormat = DXGI_FORMAT_R8G8B8A8_UNORM) :
            m_enabled(true),
            m_epoch(0)
        {
        ...
        virtual void SetInput(const wchar_t* pName, const wchar_t* pTextureName, const wchar_t* pSamplerName, IGPUPipelineNode*pNode) override;
        virtual void SetOutputSize(uint32_t width, uint32_t height) override { m_width = width; m_height = height; }
        virtual uint32_t GetOutputWidth() override { return m_width; }
        virtual uint32_t GetOutputHeight() override { return m_height; }
    ...
    {
    public:
        CGPUPipelineNode(uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) : CGPUPipelineNodeBase(outputWidth, outputHeight, format)
        {
            m_cpuFlags = (D3D11_CPU_ACCESS_FLAG)0;
    ...
    class CGPUPipelineDepthMeshNode : public CGPUPipelineNodeBase
    {
        ...
        uint32_t m_epoch; // Epoch when this was last run
    public:
        CGPUPipelineDepthMeshNode(IRenderer* pRenderer, IShader *pShader, uint32_t depthInputWidth, uint32_t depthInputHeight,
            uint32_t outputColorWidth, uint32_t outputColorHeight, CRefObj<ITexture> spRayTex, Matrix4x4& extrinsics, CameraIntrinsics& intrinsics,
    ...
    class CGPUPipelineSourceNode : public CGPUPipelineNodeBase, public IGPUPipelineSourceNode
    {
        ...
        int m_index;
    public:
        CGPUPipelineSourceNode(uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) : CGPUPipelineNodeBase(outputWidth, outputHeight, format)
        {
    ...
    public:
        CGPUPipelineSinkNode(uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) : CGPUPipelineNodeBase(outputWidth, outputHeight, format)
        {
            m_cpuFlags = (D3D11_CPU_ACCESS_FLAG)0;
            m_bindFlags = (D3D11_BIND_FLAG)(D3D11_BIND_FLAG::D3D11_BIND_RENDER_TARGET | D3D11_BIND_FLAG::D3D11_BIND_SHADER_RESOURCE);
        }
    ...
    public:
        ...
        virtual CRefObj<IGPUPipelineSourceNode> CreateSourceNode(const wchar_t* pName, IImage *pImage, uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) override;
        virtual CRefObj<IGPUPipelineSinkNode> CreateSinkNode(const wchar_t* pName, IShader *pShader, uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) override;
        virtual CRefObj<IGPUPipelineNode> CreateNode(const wchar_t* pName, IShader *pShader, uint32_t outputWidth, uint32_t outputHeight, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM) override;
        ...
        virtual uint32_t GetCurrentEpoch() override { return m_epoch; }
    };
