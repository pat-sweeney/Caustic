...
namespace Caustic
{
	...
	struct IShader : public IRefCount
	{
		...
		virtual void BeginRender(IRenderer *pRenderer, IRenderMaterial *pFrontMaterial, IRenderMaterial *pBackMaterial, std::vector<CRefObj<IPointLight>> &lights, DirectX::XMMATRIX *pWorld = nullptr) = 0; // Called before rendering. This method sets the device with the shader
